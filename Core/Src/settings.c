...
/* void checksumError(uint8_t mode){ */
/* 	Lcd_error_init(); */
/* 	ucg_WriteString(&ucg, 1, 1, "BAD CHECKSUM"); */
/* 	ucg_WriteString(&ucg, 1, 20, "RESTORING THE"); */
/*   if(mode==reset_Profile){ */
/* 	ucg_WriteString(&ucg, 1, 39, "PROFILE"); */
/*   } */
/*   else{ */
/* 	ucg_WriteString(&ucg, 1, 39, "SYSTEM"); */
/*   } */
...
/* 	if(!BUTTON_input()){ */
/* 		Lcd_error_init(); */
/* 		ucg_WriteString(&ucg, 1, 10, "HOLD BUTTON"); */
/* 		ucg_WriteString(&ucg, 1, 29, "TO RESTORE"); */
/* 		ucg_WriteString(&ucg, 1, 42, "DEFAULTS"); */
/* 		while(!BUTTON_input()){ */
/* 			#<{(| HAL_IWDG_Refresh(&hiwdg); |)}># */
/* 			if((HAL_GetTick()-ResetTimer)>5000){ */
/* 				ucg_SetBackColor(&ucg, C_BLACK); */
/* 				ucg_FillScreen(&ucg); */
/* 				ucg_WriteString(&ucg, 1, 16, "RELEASE"); */
/* 				ucg_WriteString(&ucg, 1, 32, "BUTTON NOW"); */
/* 				while(!BUTTON_input()){ */
/* 					#<{(| HAL_IWDG_Refresh(&hiwdg); |)}># */
/* 				} */
/* 				resetSystemSettings(); */
...
/* 	ucg_DrawRectangle(&ucg, xpos, ypos, ucg_GetStrWidth(&ucg, &font_18m, str), font_18m.chars[0].image->height); */
/* 	ucg_SetForeColor(&ucg, C_RED); */
...
void resetSettings() {
	...
	strcpy(systemSettings.ironTips[0].name, "KU");
#ifdef JBC
#warning "BUILDING FOR JBC"
	...
	strcpy(systemSettings.ironTips[1].name, "K\0ÿ");
	...
	strcpy(systemSettings.ironTips[2].name, "BL\0ÿ");
	...
	strcpy(systemSettings.ironTips[3].name, "ILS\0ÿ");
